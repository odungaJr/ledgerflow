...
- Decide on an actual hosting target (VPS, Fly.io, Render, etc.) and set real production secrets — the Docker setup is host-agnostic but nothing is deployed anywhere yet
- Consider basic auth/access control before this goes anywhere beyond localhost — every endpoint is currently open
